...
class Galaxies(ABC):
    ...
    def precompute(self, nside, pix_todo, z_grid, names=None, weights=None, lambda_cosmo={"H0": 70, "Om0": 0.3, "Ok0":0.0}):
    ...
    def get_interpolant(self, dir_interp=None, z_range=[0.073, 1.3]):
        ...
        def _generic_interpolant(z):
            p_cat_int = np.where((z>z_range[0])&(z<z_range[1]), LCDM.dV_dz(z, {"H0": 70, "Om0": 0.3, "Ok0":0.0}), 0)
            p_cat_int /= _fR({"H0": 70, "Om0": 0.3, "Ok0":0.0})
            return p_cat_int
        ...
        if dir_interp is not None:
            ...
            log.info("No catalog interpolant provided, using dVdz(70,0.3,0)")
        ...
        def _fR(lambda_cosmo):
            return float(LCDM.V(z_range[1], lambda_cosmo)-LCDM.V(z_range[0], lambda_cosmo))
        
        def _p_bkg_fcn(z, lambda_cosmo):
            return _fR(lambda_cosmo)*p_cat_int(z) + (1-self.P_compl(z))*np.array(LCDM.dV_dz(z, lambda_cosmo))  
...
@jax.jit
def sum_Gaussians_UCV(z_grid, mu, sigma, weights=None, lambda_cosmo={"H0": 70, "Om0": 0.3, "Ok0":0.0}):
    ...
    gauss = Gaussian(z_grid, mu, sigma)*LCDM.dV_dz(z_grid, lambda_cosmo)
    norm  = jnp.trapezoid(gauss, z_grid, axis=0)
...
def deconvolve_volume_factor(z_grid, p_cat, lambda_cosmo={"H0": 70, "Om0": 0.3, "Ok0":0.0}):
    """ TBD
    """
    z_grid = np.array(z_grid)[:, np.newaxis]
    p_cat  = np.array(p_cat)

    return np.trapz(p_cat * LCDM.dV_dz(z_grid, lambda_cosmo), z_grid, axis=0)
...
def UVC_distribution(z_grid, lambda_cosmo={"H0":70,"Om0":0.3, "Ok0":0.0}):
    """ TBD
    """
    norm = LCDM.V(np.max(z_grid), lambda_cosmo) - LCDM.V(np.min(z_grid), lambda_cosmo)
    return 4*np.pi*LCDM.dV_dz(z_grid, lambda_cosmo) / norm
