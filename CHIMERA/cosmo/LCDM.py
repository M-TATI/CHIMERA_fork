...
@jit
def E_inv_norel(z, Om0,Ok0):
    return np.power((1. + z)**3 * Om0 + Ok0 * (1. + z)**2 + (1-Om0-Ok0), -0.5)
...
@jit
def _int_dC_hyperbolic(z, Om0, Ok0):
    s = ((1-Om0-Ok0) / Om0) ** (1./3)
    prefactor = (s * Om0)**(-0.5)
...
def dC(z, args):
    ...
    return c_light / (1e3 * args["H0"]) * _int_dC_hyperbolic(z, args["Om0"], args["Ok0"])
...
def ddL_dz(z, args, dL=None):
    ...
    if dL is not None:
        ...
        return dL / (1 + z) + c_light / (1e3 * args["H0"]) * (1 + z) * E_inv_norel(z, args["Om0"], args["Ok0"])
    
    return dC(z, args) + c_light / (1e3 * args["H0"]) * (1 + z) * E_inv_norel(z, args["Om0"], args["Ok0"])
...
def dV_dz(z, args):
    ...
    return 4 * np.pi * c_light / (1e3 * args["H0"]) * (dC(z, args) ** 2) * E_inv_norel(z, args["Om0"], args["Ok0"])
