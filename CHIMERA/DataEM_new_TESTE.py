...
from CHIMERA.utils import (presets_new, mags)
    ...
    def load(self, 
        ...
        if Lcut is not None:
            ...
            if colL not in data.keys():
                ...
                dL   = LCDM.dL(data["z"], presets_new.lambda_cosmo_GLADE)
...
def cut_dataEM_given_dataGW(dataGAL, dataGWi, init_sky_cut=4, dL_cut=None, H0_prior_range=None, conf_level_KDE=None, add_cat_keys=[]):
    ...
    if dL_cut is not None:
        ...
        zmin = LCDM.z_from_dL(dmin, {"H0":H0_prior_range[0], "Om0":0.3, "Ok0":0.0})
        zmax = LCDM.z_from_dL(dmax, {"H0":H0_prior_range[1], "Om0":0.3, "Ok0":0.0})
...
class GeneralGalaxyCatalog:
    """Generic class to handle galaxy catalogs.

    Args:
        dir_cat (str): Path to the file containing the catalog.
        format (str, optional): Format of the catalog (e.g., 'hdf5', 'csv', etc.). Defaults to 'hdf5'.
        nside (int, optional): Nside of the healpix map. Defaults to None.
    """
    def __init__(self, dir_cat, format='hdf5', nside=None, **kwargs):
        self.dir_cat = dir_cat
        self.format = format
        self.nside = nside
        self.data = None
    ...
    def load(self, keys=None, filters=None):
        """Load the catalog data and apply optional filters.

        Args:
            keys (list, optional): List of keys to load from the catalog. Defaults to None.
            filters (dict, optional): Dictionary of filters to apply (e.g., {key: condition}). Defaults to None.
        """
        if self.format == 'hdf5':
            self.data = self._load_hdf5(keys)
        elif self.format == 'csv':
            self.data = self._load_csv(keys)
        else:
    ...
    def _load_csv(self, keys):
        """Load data from a CSV file."""
        import pandas as pd
        df = pd.read_csv(self.dir_cat)
        if keys is not None:
            df = df[keys]
    ...
    def _apply_filters(self, filters):
        ...
        mask = np.ones(len(next(iter(self.data.values()))), dtype=bool)
        for key, condition in filters.items():
            mask &= condition(self.data[key])

        for key in self.data.keys():
            self.data[key] = self.data[key][mask]

        log.info(f"Filters applied. Remaining data size: {mask.sum()}")

    def compute_property(self, key, func, *args, **kwargs):
        """Compute a new property for the catalog.

        Args:
            key (str): Name of the new property.
            func (callable): Function to compute the property.
            *args: Positional arguments for the function.
            **kwargs: Keyword arguments for the function.
        """
        self.data[key] = func(self.data, *args, **kwargs)
