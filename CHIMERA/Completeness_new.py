...
from CHIMERA.utils import angles, presets_new
...
lambda_cosmo = {"H0":70.0, "Om0":0.3, "Ok0":0}
...
class EmptyCatalog(Completeness):
    ...
    def get_fR(self, lambda_cosmo, z_res = 1000, z_det_range=[0,20]):
        ...
        fR = np.trapz(np.array(LCDM.dV_dz(zz, lambda_cosmo)), zz)
...
class MaskCompleteness():
    ...
    def _compute_coarse(self):
        ...
        for i in range(self.N_masks):
            ...
            if sum(in_gal) > 0:
                volMpc = 1e9 * area * np.array((LCDM.dC(self.z_edges[1:], presets_new.lambda_cosmo_737)**3 -\
                                       LCDM.dC(self.z_edges[:-1], presets_new.lambda_cosmo_737)**3)/3)
    ...
    def get_fR(self, lambda_cosmo, z_res = 1000, z_det_range=[0,20]):
        ...
        for i_mask in range(self.N_masks):
            ...
            fR_array[i_mask] = np.trapz(P_compl*np.array(LCDM.dV_dz(zz, lambda_cosmo)), zz) / np.trapz(np.array(LCDM.dV_dz(zz, lambda_cosmo)), zz)
